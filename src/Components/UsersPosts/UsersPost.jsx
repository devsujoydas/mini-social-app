...
const UsersPost = ({ post }) => {
  const { userData, postsData, setPostsData, savePostHandler, allUsers } = useContext(AuthContext);
  ...
  const reactorsUsers = post.likes.map(id => allUsers.find(u => u._id === id)).filter(Boolean);
    ...
    <div className="shadow-xl border-t border-zinc-300 md:w-full rounded-2xl bg-white">
      ...
      <div className="md:px-5 md:py-3 p-3 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <h1 className="font-semibold text-md">{post.author?.name || "Unknown"}</h1>
        ...
        <div className="relative">
          ...
          {showEdit && (
            <div onMouseLeave={() => setShowEdit(false)}
              ...
              <button onClick={sharePostHandler} className="w-full flex items-center gap-2 hover:bg-zinc-200 p-2 rounded-md">
                <FaCopy /> Copy URL
              </button>
              <Link to={`/post/update/${post._id}`} className="w-full flex items-center gap-2 hover:bg-zinc-200 p-2 rounded-md">
                <MdEdit /> Edit Post
              </Link>
              <button onClick={() => savePostHandler(post)} className="w-full flex items-center gap-2 hover:bg-zinc-200 p-2 rounded-md">
              ...
              <button onClick={deletePost} className="w-full flex items-center gap-2 text-red-600 hover:bg-red-100 p-2 rounded-md">
                <FaTrashCan /> Delete Post
              </button>
            </div>
          )}
        </div>
      ...
      <div className="md:px-5 space-y-2">
        {post.content?.text && <p className="px-2 pt-2 text-sm font-semibold">{post.content.text}</p>}
        {post.content?.postImageUrl && (
          <Link to={`/post/${post._id}`}>
            <img src={post.content.postImageUrl} alt="Post" className="w-full object-cover rounded-lg md:h-[550px] h-56" />
          </Link>
        )}
        ...
        <div className="flex justify-between items-center my-1">
          <div className="flex items-center md:gap-2">
            ...
            <button className={likeCommentStyle}>
              <BiCommentDots className="text-xl" /> <span className="hidden md:flex">Comments</span>
            </button>
            <button onClick={sharePostHandler} className={likeCommentStyle}>
              <PiShareFatBold className="text-xl" /> <span className="hidden md:flex">Shares</span>
            </button>
          </div>
          <CiBookmark onClick={() => savePostHandler(post)} className="text-2xl cursor-pointer active:scale-95 transition-all" />
        </div>
      </div>
      ...
      <form className="p-3 md:p-4 flex justify-between items-center gap-2 md:gap-20">
        <div className="flex items-center gap-2 md:gap-4 w-full">
          <Link to="/profile">
            <img src={userData?.profilephotourl || "/default.jpg"} alt={userData?.name} className="w-10 h-10 md:w-12 md:h-12 rounded-full object-cover" />
          </Link>
          <input type="text" placeholder="Write your comment..." className="w-full border border-zinc-400 outline-none text-xs md:text-sm py-2 md:py-3 px-2 md:px-4 rounded-full" />
        </div>
        <div className="flex items-center gap-3">
          <ImAttachment className="border border-zinc-400 p-2 md:p-3 rounded-full cursor-pointer active:scale-95 hover:bg-zinc-200" />
          <FaRegSmile className="border border-zinc-400 p-2 md:p-3 rounded-full cursor-pointer active:scale-95 hover:bg-zinc-200" />
          <VscSend className="border border-blue-700 text-blue-700 p-2 md:p-3 rounded-full cursor-pointer active:scale-95 hover:bg-blue-600" />
        </div>
      </form>
    </div>
  );
...
//   useEffect(() => {
//     if (userData?._id) {
//       const alreadyLiked = post.likes.some((u) => u._id === userData._id);
...
//           prev.filter((u) => u._id !== userData._id)
//         );
//         toast.success("Disliked!");
//       }
//     } catch (err) {
...
//             if (res.data.deletedCount > 0) {
//               toast.success("Post deleted!");
//               const remaining = postsData.filter((p) => p._id !== post._id);
