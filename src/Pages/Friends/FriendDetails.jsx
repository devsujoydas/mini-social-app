...
const FriendDetails = () => {
    const { user, youMayKnowFriends, addFriendBtnHanlder, unFriendBtnHanlder, confrimFriendBtnHanlder, cancelFriendRequestBtnHanlder, loading } = useContext(AuthContext);
    ...
    const isFriend = user?.myFriends?.includes(friend._id);
    const hasSentRequest = user?.sentRequests?.includes(friend._id);
    const hasReceivedRequest = user?.friendRequests?.includes(friend._id);
                ...
                <div className='lg:col-span-6 p-3 md:p-5'>
                    {loading ? <Loading /> : (
                        <div>
                            {/* Friend Details */}
                            <div className='mb-5'>
                                <div className='md:h-96 rounded-lg border border-zinc-300 h-50 md:mt-0 mt-14 bg-no-repeat bg-cover bg-center' style={{ backgroundImage: `url(${friend?.coverphotourl || "https://www.deped.gov.ph/wp-content/uploads/placeholder.png"})` }}>
                                </div>

                                <div className='flex md:gap-5 items-center'>
                                    <div className='md:ml-10 ml-2'>
                                        <div className='md:w-50 w-26 md:h-50 outline outline-zinc-300 bg-white h-26 border-4 border-white md:-mt-23 -mt-14 rounded-full bg-no-repeat bg-cover bg-center' style={{ backgroundImage: `url(${friend?.profilephotourl || "/default.jpg"})` }}></div>
                                    </div>
                                    ...
                                    <div className='flex items-center flex-col md:flex-row md:gap-5'>
                                        ...
                                        <div className='md:mt-4 mt-2 flex justify-center items-center flex-wrap gap-2'>
                                            ...
                                            {isFriend && (
                                                <button onClick={unfriendHandler} className={`${btnStyle} bg-red-100 text-red-600 font-semibold`}>
                                                    <RiUserSharedFill className='md:text-lg text-sm' /> Unfriend
                                                </button>
                                            )}

                                            {!isFriend && hasReceivedRequest && (
                                                <button onClick={confirmFriendHandler} className={`${btnStyle} bg-green-100 text-green-700 font-semibold`}>
                                                    ✅ Confirm
                                                </button>
                                            )}

                                            {!isFriend && hasSentRequest && (
                                                <button onClick={cancelRequestHandler} className={`${btnStyle} bg-yellow-100 text-yellow-700 font-semibold`}>
                                                    ❌ Cancel
                                                </button>
                                            )}

                                            {!isFriend && !hasSentRequest && !hasReceivedRequest && (
                                                <button onClick={addFriendHandler} className={`${btnStyle} bg-blue-200 text-blue-800 font-semibold flex items-center md:gap-2 gap-1`}>
                                                    <FaUserPlus className='md:text-lg text-sm' /> Add Friend
                                                </button>
                                            )}

                                            <Link to={`/message/${friend?.username}`} className='follow-btn'>
